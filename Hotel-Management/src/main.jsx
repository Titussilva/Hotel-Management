...
function useApi(path, fallback, token, refreshTick = 0) {
...
function App() {
  ...
  const [paymentMethod, setPaymentMethod] = useState('pay_by_email');
  ...
  const [reviewStatus, setReviewStatus] = useState({ message: '', isError: false });
  ...
  const { data: rooms, loading } = useApi(`/rooms?${query}`, fallbackRooms, session?.token, roomsRefreshTick);
  const { data: offers } = useApi('/offers', fallbackOffers, session?.token, offersRefreshTick);
  ...
  async function refreshAdminData() {
    try {
      ...
      console.log('[Admin] Data refreshed:', { bookings: adminBookings.length, reviews: adminReviews.length });
      setAdminData({ bookings: adminBookings, reviews: adminReviews, analytics });
      setRoomsRefreshTick((t) => t + 1);
      setOffersRefreshTick((t) => t + 1);
    } catch (error) {
      console.error('[Admin] refreshAdminData failed:', error.message);
      setStatus(error.message || 'Admin data could not be refreshed. Check your session.');
    }
  }
  ...
  async function createBooking(method = paymentMethod) {
    if (String(activeRoom._id).startsWith('demo-') || session.token === 'demo-token') {
      const demoBooking = {
      ...
      setBookings((current) => [demoBooking, ...current]);
      setNotifications((current) => [
        { _id: `demo-note-${Date.now()}`, title: 'Booking confirmed', message: `${activeRoom.name} is reserved.`, read: false },
        ...current,
      ]);
      setStatus(`${activeRoom.name} booking request saved in demo mode. Booking details are ready to send by Gmail. Payment status: pending.`);
      return null;
    }
  ...
  async function submitReview(event) {
    ...
    if (!reviewForm.comment.trim()) {
      setReviewStatus({ message: 'Write a review comment first.', isError: true });
      return;
    }
    ...
    if (session.token === 'demo-token' || String(activeRoom._id).startsWith('demo-')) {
      ...
      setReviewStatus({ message: 'Review submitted in demo mode.', isError: false });
      return;
    }
    ...
    try {
      ...
      console.log('[Review] Submitting review, body:', body);
      const saved = await apiRequest('/reviews', { method: 'POST', token: session.token, body });
      console.log('[Review] Review saved in DB:', saved);
      // Optimistically add review to local list so user sees it immediately.
      // Backend stores as 'pending'; rooms/:id only returns 'approved' reviews,
      // so we add it locally to give the guest immediate visual confirmation.
      setReviews((current) => [
        {
          _id: saved._id || `pending-${Date.now()}`,
          rating: Number(reviewForm.rating),
          title: reviewForm.title || 'Guest review',
          comment: reviewForm.comment,
          user: { name: session.user.name },
      ...
      setReviewForm({ rating: 5, title: '', comment: '' });
      setReviewStatus({ message: 'Review submitted — pending admin approval.', isError: false });
    } catch (error) {
      console.error('[Review] submitReview failed:', error.message);
      setReviewStatus({ message: error.message || 'Review could not be submitted.', isError: true });
    }
  }
  ...
  async function createAdminRoom(event) {
    ...
    try {
      ...
      console.log('[Admin] Creating room, payload:', payload);
      const result = await apiRequest('/admin/rooms', {
        method: 'POST',
        token: session.token,
        body: payload,
      });
      console.log('[Admin] Room created, response:', result);
      setStatus('Admin room created.');
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] createAdminRoom failed:', error.message);
  ...
  async function updateAdminRoom(room) {
    try {
      ...
      console.log('[Admin] Updating room', room._id, 'payload:', payload);
      const result = await apiRequest(`/admin/rooms/${room._id}`, {
        method: 'PUT',
        token: session.token,
        body: payload,
      });
      console.log('[Admin] Room updated, response:', result);
      setStatus(`${room.name} updated by admin.`);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] updateAdminRoom failed:', error.message);
  ...
  async function removeAdminRoom(room) {
    try {
      console.log('[Admin] Removing room:', room._id, room.name);
      const result = await apiRequest(`/admin/rooms/${room._id}`, { method: 'DELETE', token: session.token });
      console.log('[Admin] Room removed, response:', result);
      setStatus(`${room.name} removed from active listings.`);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] removeAdminRoom failed:', error.message);
  ...
  async function createAdminOffer(event) {
    ...
    try {
      ...
      console.log('[Admin] Creating offer, payload:', payload);
      const result = await apiRequest('/admin/offers', {
        method: 'POST',
        token: session.token,
        body: payload,
      });
      console.log('[Admin] Offer created, response:', result);
      setStatus('Admin offer created.');
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] createAdminOffer failed:', error.message);
  ...
  async function updateAdminOffer(offer) {
    try {
      const payload = { title: `${offer.title} Updated`, discountValue: Number(offer.discountValue || 0) + 1 };
      console.log('[Admin] Updating offer', offer._id, 'payload:', payload);
      const result = await apiRequest(`/admin/offers/${offer._id}`, { method: 'PUT', token: session.token, body: payload });
      console.log('[Admin] Offer updated, response:', result);
      setStatus(`${offer.code} updated by admin.`);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] updateAdminOffer failed:', error.message);
  ...
  async function removeAdminOffer(offer) {
    try {
      console.log('[Admin] Removing offer:', offer._id, offer.code);
      const result = await apiRequest(`/admin/offers/${offer._id}`, { method: 'DELETE', token: session.token });
      console.log('[Admin] Offer removed, response:', result);
      setStatus(`${offer.code} disabled.`);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] removeAdminOffer failed:', error.message);
  ...
  async function updateAdminBooking(bookingId, statusValue) {
    try {
      console.log('[Admin] Updating booking', bookingId, 'status ->', statusValue);
      const result = await apiRequest(`/admin/bookings/${bookingId}`, { method: 'PATCH', token: session.token, body: { status: statusValue } });
      console.log('[Admin] Booking updated, response:', result);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] updateAdminBooking failed:', error.message);
      setStatus(error.message || 'Booking could not be updated.');
    }
  }

  async function moderateReview(reviewId, statusValue) {
    try {
      console.log('[Admin] Moderating review', reviewId, 'status ->', statusValue);
      const result = await apiRequest(`/admin/reviews/${reviewId}`, { method: 'PATCH', token: session.token, body: { status: statusValue } });
      console.log('[Admin] Review moderated, response:', result);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] moderateReview failed:', error.message);
      setStatus(error.message || 'Review could not be moderated.');
    }
  }

  async function respondToReview(reviewId) {
    try {
      console.log('[Admin] Responding to review:', reviewId);
      const result = await apiRequest(`/admin/reviews/${reviewId}`, {
        method: 'PATCH',
        token: session.token,
        body: { status: 'approved', adminResponse: 'Thank you for the feedback. Our team has reviewed your stay experience.' },
      });
      console.log('[Admin] Review response saved, response:', result);
      refreshAdminData();
    } catch (error) {
      console.error('[Admin] respondToReview failed:', error.message);
      setStatus(error.message || 'Review response could not be saved.');
    }
  }
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-5 sm:px-8 lg:grid-cols-[1fr_420px]">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Select
                ...
                options={[
                  ...
                  { value: 'pay_by_email', label: 'Pay after email confirmation' },
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                ...
                <button className="self-end rounded-md bg-coral px-5 py-3 font-semibold text-white disabled:cursor-not-allowed disabled:bg-slate-300" disabled={isPaying}>
                  {isPaying ? 'Sending...' : paymentMethod === 'pay_at_hotel' ? 'Reserve room' : 'Send booking'}
                </button>
              </div>
            </div>
          ...
          <aside className="rounded-lg bg-ink p-5 text-white">
            ...
            <div className="mt-6 rounded-md bg-white/10 p-4 text-sm text-white/78">
              Selected method: {paymentLabel(paymentMethod)}. The app records the payment option and emails booking details for secure follow-up.
            </div>
          </aside>
        </div>
      </section>
        ...
        <Panel icon={<MessageSquareText />} title="Guest reviews">
          <form onSubmit={submitReview}>
            ...
            {reviewStatus.message && (
              <p className={`mt-2 rounded-md px-3 py-2 text-sm font-medium ${reviewStatus.isError ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}>
                {reviewStatus.message}
              </p>
            )}
            <button className="mt-3 rounded-md bg-pine px-4 py-2 text-sm font-semibold text-white" type="submit">Submit review</button>
          </form>
