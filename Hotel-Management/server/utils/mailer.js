...
// Log Gmail credential detection once at module load time.
const _gmailUser = (process.env.GMAIL_USER || '').trim();
const _gmailPass = (process.env.GMAIL_APP_PASSWORD || '').trim();
if (_gmailUser && _gmailPass && !_gmailPass.includes('replace_me')) {
  console.log(`[mailer] Gmail credentials detected for: ${_gmailUser}`);
} else {
  console.warn('[mailer] Gmail credentials NOT detected – emails will not be sent.');
  console.warn(`[mailer] GMAIL_USER=${_gmailUser ? '"' + _gmailUser + '"' : '(empty)'}, GMAIL_APP_PASSWORD=${_gmailPass ? '(set)' : '(empty)'}`);
}
...
function getTransporter() {
  ...
  if (!user || !pass || pass.includes('replace_me')) return null;
...
export async function sendBookingEmail({ booking, room, user, recipient }) {
  ...
  try {
    ...
    return { sent: false, to, cc, reason: error.message || 'Unknown email error' };
  }
